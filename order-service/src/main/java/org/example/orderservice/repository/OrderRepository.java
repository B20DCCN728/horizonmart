...
public interface OrderRepository extends JpaRepository<Order, Long> {
    @Query(
            "SELECT SUM(op.quantity * op.sellingPrice) AS total_revenue " +
                    "FROM Order o " +
    ...
    @Query(
            "SELECT SUM(op.quantity) AS total_sold " +
                    "FROM Order o " +
                    "JOIN OrderProduct op " +
                    "ON o.id = op.order.id " +
                    "WHERE o.orderDate >= ?1 " +
    ...
    @Query(
            "SELECT op.productId FROM Order o " +
                    "JOIN OrderProduct op " +
                    "ON o.id = op.order.id " +
                    "WHERE o.orderDate >= ?1 " +
                    "AND o.orderDate <= ?2 " +
    ...
    List<Order> getTop10HighestOrderValue();
}
